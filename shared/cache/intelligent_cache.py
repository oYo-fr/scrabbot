...
class IntelligentCache:
    ...
    def _record_access_pattern(self, key: str) -> None:
        ...
        self._access_pattern[key] = [
            t for t in self._access_pattern[key] if t > hour_ago
        ]
    ...
    def _update_access_time(self, elapsed_ms: float) -> None:
        ...
        if total_accesses > 0:
            self.stats.average_access_time_ms = (
                (self.stats.average_access_time_ms * (total_accesses - 1) + elapsed_ms) 
                / total_accesses
            )
    ...
    def optimize_cache(self) -> Dict[str, Any]:
        ...
        active_entries = len([
            key for key, entry in self._cache.items()
            if entry.access_count > 1 and (time.time() - entry.last_access) < 1800
        ])
...
class DictionaryCacheManager:
    ...
    def get_combined_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_memory_usage_estimate": (
                self.word_cache.stats.size + 
                self.definition_cache.stats.size + 
                self.search_cache.stats.size
            )
        }
