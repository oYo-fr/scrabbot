...
class DictionaryAnalytics:
    ...
    def analyze_language_statistics(self, language: str, force_refresh: bool = False) -> LanguageStatistics:
        ...
        try:
            with sqlite3.connect(db_path) as conn:
                ...
                cursor.execute(f"SELECT AVG(longueur) FROM {table_name}" if language == "fr" else f"SELECT AVG(length) FROM {table_name}")
        ...
        letter_rarity = {letter: max_freq / freq for letter, freq in letter_frequency.items()}
    ...
    def _generate_scrabble_insights(self, words_data: List[Tuple], language: str) -> Dict[str, Any]:
        ...
        short_high_value = [(word, points) for word, points, length, _, _ in words_data if 2 <= length <= 4 and points >= 8]
        ...
        q_without_u = [word for word, _, _, _, _ in words_data if "Q" in word and "U" not in word]
        ...
        for ending in common_endings:
            ...
            matching_words = [word for word, _, _, _, _ in words_data if word.endswith(suffix)]
        ...
        comparison = {
            "basic_comparison": {
                ...
                "average_word_length": {
                    ...
                    "difference": round(
                        abs(fr_stats.average_word_length - en_stats.average_word_length),
                        2,
                    ),
                },
            },
            "letter_frequency_comparison": self._compare_letter_frequencies(fr_stats, en_stats),
            "length_distribution_comparison": self._compare_distributions(fr_stats.length_distribution, en_stats.length_distribution),
    ...
    def _compare_letter_frequencies(self, fr_stats: LanguageStatistics, en_stats: LanguageStatistics) -> Dict[str, Any]:
        ...
        biggest_diffs = sorted(differences.items(), key=lambda x: x[1]["difference"], reverse=True)[:10]
    ...
    def export_analytics_report(self, output_file: str = "dictionary_analytics_report.json"):
        ...
        report = {
            ...
            "french_word_patterns": [self._pattern_to_dict(p) for p in self.analyze_word_patterns("fr")[:20]],
            "english_word_patterns": [self._pattern_to_dict(p) for p in self.analyze_word_patterns("en")[:20]],
