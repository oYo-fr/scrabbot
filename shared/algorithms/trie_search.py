...
class AdvancedSearchEngine:
    ...
    def _pattern_search_recursive(self, node: TrieNode, pattern: str, pattern_idx: int, 
                                 current_word: str, results: List[str], max_results: int):
        ...
        if char == '?':
            ...
            for letter, child in node.children.items():
                self._pattern_search_recursive(
                    child, pattern, pattern_idx + 1, 
                    current_word + letter, results, max_results
                )
        elif char == '*':
            # Any sequence of letters (including empty)
            # Skip the * and try matching the rest
            self._pattern_search_recursive(
                node, pattern, pattern_idx + 1, 
                current_word, results, max_results
            )
            # Or consume one letter and try again
            for letter, child in node.children.items():
                self._pattern_search_recursive(
                    child, pattern, pattern_idx, 
                    current_word + letter, results, max_results
                )
        else:
            # Exact letter match
            if char in node.children:
                self._pattern_search_recursive(
                    node.children[char], pattern, pattern_idx + 1,
                    current_word + char, results, max_results
                )
